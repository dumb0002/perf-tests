...
func verifyCluster(c kubernetes.Interface) error {
	if clusterLoaderConfig.ClusterConfig.Provider.Name() == provider.KCPName || clusterLoaderConfig.ClusterConfig.Provider.Name() == provider.KSName {
		return nil
	}
...
func main() {
	...
	if !dryRun {
		...
		if clusterLoaderConfig.ClusterConfig.Provider.Name() != provider.KSName {
